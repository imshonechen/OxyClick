...
const CURSOR_MOVE_REPAINT_INTERVAL: Duration = Duration::from_millis(50);
...
pub struct EpiIntegration {
    ...
    last_cursor_move_repaint: Instant,
...
impl EpiIntegration {
    ...
    pub fn new(
        ...
        Self {
            ...
            last_cursor_move_repaint: Instant::now(),
    ...
    pub fn on_window_event(
            ...
            WindowEvent::MouseInput {
                ...
                match state {
                    ElementState::Pressed => {
                        self.pressed_mouse_buttons = self.pressed_mouse_buttons.saturating_add(1);
                        self.can_drag_window = true;
                    }
                    ElementState::Released => {
                        self.pressed_mouse_buttons = self.pressed_mouse_buttons.saturating_sub(1);
                    }
                }
            }
        ...
        let mut response = egui_winit.on_window_event(window, event);
        ...
        {
            let now = Instant::now();
            let elapsed = now.saturating_duration_since(self.last_cursor_move_repaint);

            if elapsed < CURSOR_MOVE_REPAINT_INTERVAL {
                self.egui_ctx
                    .request_repaint_after(CURSOR_MOVE_REPAINT_INTERVAL - elapsed);
                response.repaint = false;
            } else {
                self.last_cursor_move_repaint = now;
            }
        }
